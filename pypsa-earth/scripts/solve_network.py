...
def extra_functionality(n, snapshots):
    ...
    if cb_cfg.get("enable", False):
        carrier = cb_cfg.get("carrier", "cross-border")
        # read ntc_gw from config, default to 2.6 if missing
        ntc_gw = float(cb_cfg.get("ntc_gw", 0))
        ntc_mw = ntc_gw * 1000.0

        # select cross-border generators
        gens_mask = n.generators.carrier.astype(str) == carrier
        gens = n.generators.index[gens_mask]
        ...
        if len(gens) == 0:
            ...
            m = n.model  # linopy model

            # Generator dispatch variable (MW), dims: snapshot × name
            p_gen = m.variables["Generator-p"].sel(snapshot=snapshots, Generator=gens)

            # Sum over generators -> expression per snapshot
            expr = p_gen.sum("Generator")  # indexed by 'snapshot'

            # Bidirectional capacity limit for each snapshot:
            #   -ntc_mw <= sum_g p_gen(t,g) <= ntc_mw
            m.add_constraints(
                expr <= ntc_mw,
                name="max_cross_border_plus",
            )
            m.add_constraints(
                expr >= -ntc_mw,
                name="max_cross_border_minus",
            )

            logger.info(
                f"[extra_functionality] Added per-snapshot cross-border limit "
                f"±{ntc_mw:.1f} MW for carrier {carrier!r}."
            )
    # -----------------------------------------------------------------------
    ...
    if "CCL" in opts and n.generators.p_nom_extendable.any():
        add_CCL_constraints(n, config)
    # ... rest of your existing extra_functionality body ...
    add_co2_sequestration_limit(n, snapshots)
